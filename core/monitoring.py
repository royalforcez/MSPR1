...
def get_ssh_metrics(ip, username):
    ...
    try:
        ...
        res_linux = stdout.read().decode().strip().splitlines()
        ...
        if len(res_linux) >= 8 and "non reconnu" not in res_linux[0].lower() and "not recognized" not in res_linux[0].lower():
            # On sécurise la conversion en nombre (si top renvoie un texte vide, on met 0)
            cpu_val = res_linux[1].strip()
            cpu_final = float(cpu_val) if cpu_val else 0.0

            return {
                "host": res_linux[0].strip(), 
                "cpu": cpu_final, 
                "ram": float(res_linux[2]), 
                "disk": float(res_linux[3]),
                "uptime": res_linux[4], 
        ...
        # 2. TENTATIVE WINDOWS POWERSHELL
        cmd_win = """powershell -Command "$cpu=(Get-CimInstance Win32_Processor).LoadPercentage; if ($cpu -eq $null) {$cpu=0}; $os=Get-CimInstance Win32_OperatingSystem; $ram=[math]::Round((($os.TotalVisibleMemorySize - $os.FreePhysicalMemory) / $os.TotalVisibleMemorySize) * 100, 2); $disk=Get-CimInstance Win32_LogicalDisk -Filter \\"DeviceID='C:'\\"; $disk_p=[math]::Round((($disk.Size - $disk.FreeSpace) / $disk.Size) * 100, 2); $up=(New-TimeSpan -Start $os.LastBootUpTime -End (Get-Date)).Hours; $sn=(Get-CimInstance Win32_BIOS).SerialNumber; Write-Output $env:COMPUTERNAME; Write-Output $cpu; Write-Output $ram; Write-Output $disk_p; Write-Output \\"up $up hours\\"; Write-Output $os.Caption; Write-Output $os.Version; Write-Output $sn" """
        
        stdin, stdout, stderr = ssh.exec_command(cmd_win, timeout=10)
        res_win = stdout.read().decode().strip().splitlines()
        
        if len(res_win) >= 8:
            return {
                "host": res_win[0].strip(), "cpu": float(res_win[1]), "ram": float(res_win[2]), "disk": float(res_win[3]),
                "uptime": res_win[4], "os_name": res_win[5].strip(), "os_version": res_win[6].strip(), "sn": res_win[7].strip(), "os_type": "WINDOWS"
            }
