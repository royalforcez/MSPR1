import curses
import time
# Importations des modules séparés (Modèle et Vue)
from diagnostic.data_manager import get_db_data, check_db_health
...
def screen_diagnostic(stdscr):
    global cached_data, last_db_update, db_status
    
    stdscr.nodelay(True)  # Mode non-bloquant pour la fluidité
    curses.curs_set(1)    # Affiche le curseur pour la recherche
    ...
    while True:
        ...
        if not cached_data or (current_time - last_db_update > REFRESH_INTERVAL):
            # On récupère les données ET on vérifie la santé
            result = get_db_data()
            ...
            if isinstance(result, list):
                ...
                db_online = False 
                # On ne met pas à jour last_db_update ici pour retenter plus vite si KO
        ...
        if active_tab == 0:
            if db_online:
                # Filtrage dynamique (très rapide car sur le cache)
                filtered = [d for d in cached_data if search_text.lower() in d['Nom'].lower()]
                
                # Sécurité pour la navigation clavier
                ...
                # Affichage du mode dégradé (BDD Offline)
            ...
            # On passe le db_status (calculé tous les 5s) à l'interface Services
            draw_services_interface(stdscr, h, w, db_status)
        ...
        if key == 27: # ESC
            ...
            selected_row = 0
        elif 32 <= key <= 126: # Caractères alphanumériques
            search_text += chr(key)
            selected_row = 0

        time.sleep(0.05) # ~20 FPS pour un CPU au repos